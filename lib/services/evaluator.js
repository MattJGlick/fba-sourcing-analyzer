...
exports.evaluate = function evaluate(asin, callback) {
  ...
  function evaluateProduct(mwsData, keepaData, afterFee) {
    ...
    if(mwsData.weight > config.get('maxWeight') ||
        mwsData.height > config.get('maxDimension') ||
        mwsData.width > config.get('maxDimension') ||
        mwsData.length > config.get('maxDimension')) {
      buyResponse = "false";
    }
