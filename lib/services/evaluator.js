...
exports.evaluate = function evaluate(asin, buy_price, callback) {
      ...
      mwsFacade.getSalesRank(asin, function (productData) {
        if (listingData.length > 1 && productData) {
          keepaFacade.searcher(asin, function (keepaAverages) {
            ...
            var buyPrice = buy_price ? buy_price : listingData[0].Price.LandedPrice.Amount;
            
            logger.debug("Buy Price: ", buy_price)

            productData.dimensions.forEach(function(value, index) {
                productData.dimensions[index] = parseFloat(value);
            });
  ...
  function evaluateProduct(keepaAverages, productData, buyPrice, sellPrice, fees, category) {
    ...
    var extraFees = config.get("extraFees");
    var afterFee = sellPrice - fees - extraFees;
